...
# ----------------------------
INIT_TARGET_BASE_DEG = 30.0
INIT_TARGET_ELBOW_DEG = 160.0
INIT_KP = 3.0              # proportional speed gain (dps per deg error)
INIT_MAX_DPS = 10.0         # cap during init move
...
class ArmPlot(Node):
    def __init__(self):
        ...
        plt.subplots_adjust(bottom=0.18)  # room for init button
        ...
        self.init_btn_ax = self.fig.add_axes([0.32, 0.05, 0.36, 0.08])
        ...
        # Timers
        self.timer = self.create_timer(1/30.0, self.redraw)                    # ~30 Hz UI redraw (arm)
        self.timer2 = self.create_timer(1.0 / PLOT_HZ, self.redraw_telemetry)  # telemetry 10 Hz
    ...
    def cb_th1(self, msg: Float32):
        self.th1_deg = msg.data

    def cb_th2(self, msg: Float32):
        self.th2_deg = msg.data

    def cb_ee(self, msg: Point):
        self.ee = msg

    # Telemetry callbacks
    def cb_base_i(self, msg: Float32):
        self.cur_base = msg.data

    def cb_elbow_i(self, msg: Float32):
        self.cur_elbow = msg.data

    def cb_base_t(self, msg: Float32):
        self.tmp_base = msg.data

    def cb_elbow_t(self, msg: Float32):
        self.tmp_elbow = msg.data
    ...
    def cb_joy(self, joy: Joy):
        ...
        self.last_joy_time = now
        # Clamp dt to avoid giant steps on hiccups
        dt = min(dt, 1.0/15.0)
        ...
        # Current joint angles (radians) from Teensy
        t1 = math.radians(self.th1_deg)
        t2 = math.radians(self.th2_deg)
        ...
        if moving:
            ...
            if self.has_sent_motion and not self.is_holding:
                hold = Point()
                hold.x = self.th1_deg  # use actual encoder-derived angles
                hold.y = self.th2_deg
                hold.z = 0.0
                self.pub_hold.publish(hold)
                self.is_holding = True
            ...
            # Zero-speed heartbeat to defeat dropped stop frames
            self._pub_joint_speeds(0.0, 0.0, now)
